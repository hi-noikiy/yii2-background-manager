...
Vagrant.configure(2) do |config|
  ...
  # provisioners
  config.vm.provision 'shell', path: './vagrant/provision/once-as-mysql.sh', args: [options['timezone']]
  config.vm.provision 'shell', path: './vagrant/provision/once-as-vagrant.sh', args: [options['github_token']], privileged: false
  config.vm.provision 'shell', path: './vagrant/provision/always-as-mysql.sh', run: 'always'
