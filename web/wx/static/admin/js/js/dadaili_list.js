...
var vm =  new Vue({
    ...
    data:{
        myData: [],          //代理列表分页数据
        detailData: [],          //消耗详情数据
        lessnessData: [],          //下级详情页面数据
        detailIndex: '',          //详情页的index
    ...
    methods:{
        ...
        detail : function(item){
            ...
            if(item != null){
                this.curItem = JSON.parse(JSON.stringify(item));
                var player_index = this.curItem.PLAYER_INDEX;
            }else{
                var player_index = that.detailIndex;
            }
            ...
            $.ajax({
                type : 'get',
                url : base_url + '&r=admin/get-detail-list',
                data : {
                ...
                success: function(res){
                    ...
                    if( json.ret_code == 0 ){
                        if( json.data.data != 0 ){
                            ...
                            that.detailIndex = json.data.index;
                        }else{
                            that.detailData = '';
                        }
                    }
        ...
        lessness : function(item){
            ...
            var time_start = $('#lessness_time_start').val();
            var time_end = $('#lessness_time_end').val();
            console.log(player_index);
            console.log(time_start+'.'+time_end);
            var user_id = $("#user_id").val();
            $.ajax({
                type : 'get',
                url : base_url + '&r=admin/member-daili-list',
                data : {
                    player_index:player_index,
                    time_start:time_start,
                    time_end:time_end,
                ...
                success: function(res){
                    ...
                    if( json.ret_code == 0 ){
                        if( json.data.data != 0 ){
                            that.lessnessData = json.data.data;
                            that.detailIndex = json.data.index;
                        }else{
                            that.detailData = '';
                        }
        ...
        search : function(){
            ...
            $.ajax({
                type : 'get',
                url : base_url + '&r=admin/get-dadaili-list',
                data : {
                    page_index : 1 ,
