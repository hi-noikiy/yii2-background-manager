...
var vm =  new Vue({
    ...
    methods:{
        ...
        edit : function(item){
            ...
            //清空地址信息
            $('#editParentGame').val(this.curItem.game_name);
            $('#editChildGame').val(this.curItem.sub_name);
            $('#editContent').val(this.curItem.content);
            $('#edit_start').val(this.curItem.start_time);
            $('#edit_end').val(this.curItem.end_time);
            $('#edit_velocity').val(this.curItem.frequency);
            $('#edit_interval').val(this.curItem.interval_time);
            $('#edit_game').val(this.curItem.game_id);
            $('#edit_sub').val(this.curItem.sub_game_id);
            $('#edit_status').val(this.curItem.status);
            $('#edit_id').val(this.curItem.id);
            $('#edit_createtime').val(this.curItem.create_time);
        ...
        search : function(){
            ...
            $.ajax({
                type : 'get',
                url : base_url + '&r=admin-data/get-paomadeng',
                data : {
                    page_index : 1 ,
        ...
        deleteMsg:function(n){         //删除代理
            ...
            $.ajax({
                type : 'post',
                url : base_url + '&r=admin-data/del-paomadeng',
                data : { id : n },
                success : function(res){
                    var json = res;
                    if( json.ret_code == 0 ){
