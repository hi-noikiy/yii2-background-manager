...
class TestWechatController extends CommonBaseController
{
    public $Default_Page = 1;
    public $Default_Limit = 20;
    private $Daili_Access_Token = 'daili_access_token';
    private $rootUrl = 'https://oss.601yx.com';

    private $redirectUrl = 'https://oss.601yx.com/wechat/index';
...
//            $result['unionid'] = 'ooZvs0jSY-cE2SmhRDHXr8WhLqBo';
    ...
    public function withdraw($my_user)
    {
        ...
        $openid = $this->redis->hget(self::DAILI_RELATION, $my_user);
...
//            'remark' => '',
    ...
    public function actionMoneyInTime()
    {
        ...
        //获取用户所有直属下级（包括代理）
        $listAgent = [];
        $this->getIdListByPlayerId($my_user, $listAgent, 2);
...
//业绩
        ...
        return $this->writeLayui(0, 'ok', count($listAgent), $user_list ? $user_list : []);
    }
    ...
    public function actionPlayerList()
    {
        ...
        $playerList = $this->getUnderPlayerList($my_user, 2);
        $lobbyModel = new LobbyPlayer();
        $data = [];
        foreach ($playerList as $key => $val) {
            ...
            if ($playerInfo){
                ...
                $unionid = $this->redis->hget('reserved_key_id', $val);
                $this->redis->select(0);
                $info = $this->redis->hget('share_scan_info', $unionid);
        ...
        return $this->writeLayui(0, 'ok', count($data), $data ? $data : []);
    }
...
//业绩
        ...
        $this->writeLayui(0, 'ok', count($data), $data ? $data : []);
    ...
    public function actionVerifyCode()
    {
        ...
        $this->redis->select(3);
        $redis_code = $this->redis->get('daili_phone_' . $request['phone']);
...
//                $tran->rollBack();
    ...
    public function actionResultsDay()
    {
        ...
        foreach ($resDate as $key => $val) {
            ...
            $returnData[$i]['rebate'] = Common::disposeStr($this->redis->hget($dateRedisKey, $playerId) / self::REBATE_RATIO) ?: 0;
    ...
    private function getDayAgent($agent_id, $search_date)
    {
        ...
        $data = array();
        $player_list = $this->redis->zrangebyscore(RedisKey::INF_AGENT_RELATION, $agent_id, $agent_id);
        $dailiModel = new DailiPlayer();
        foreach ($player_list as $k => $player_id) {
            if ($dailiModel->getById($player_id)) {
                $data[$k] = [
                    ...
                    'id' => $player_id,
                    'consume' => round($this->redis->hget(RedisKey::INF_UNDER_DAY_CONSUME . $d, $player_id) / self::REBATE_RATIO, 2),
                ];
            }
        }
    ...
    private function getDayUser($agent_id, $search_date)
    {
        ...
        $player_list = $this->redis->zrangebyscore(RedisKey::INF_AGENT_RELATION, $agent_id, $agent_id);
        foreach ($player_list as $k => $player_id) {
            Yii::info($player_id . ";玩家消耗：：" . $this->redis->hget(RedisKey::INF_DAY_CONSUME . $d, $player_id));
            $data[$k] = [
                'nickname' => $db->createCommand("SELECT weixin_nickname FROM login_db.t_lobby_player WHERE u_id = '{$player_id}'")->queryScalar() ?: '',
                'id' => $player_id,
                'consume' => round($this->redis->hget(RedisKey::INF_DAY_CONSUME . $d, $player_id) / self::REBATE_RATIO, 2) ?: 0,
            ];
        }
        return $data;
    }
    ...
    private function getWeekAgent($agent_id, $search_date,$limit,$page)
    {
        ...
        $parentRadio = $this->redis->hget($redisKey,$agent_id);//每周的返利比例
        ...
        if ($data) {
            ...
            foreach ($data as $k => $v) {
                ...
                $data[$k]['nicknameId'] = $nickname."<br />".$v['player_id'];

                $radio = $this->redis->hget($redisKey,$v['player_id']) ?: 0.3;
    ...
    public function actionTwodimensioncode()
    {
        ...
        $QR = Yii::$app->basePath . '/web/static/mobile/agent/images/poker1.png';//二维码底图
    ...
    public function actionWeekRebateDetail()
    {
        ...
        $radio = $this->redis->hget($redisKey,$playerId);//每周的返利比例
    ...
    {
        return $this->render('resultsQuery');
    }
    ...
    public function actionTest(){
	        $parentId = $_REQUEST['playerId'];

        $data = [];
        $lowerList = [];//下级玩家列表（包括直属代理）
        $this->getIdListByPlayerId($parentId, $lowerList, 2);
        foreach ($lowerList as $key => $val) {
            $consume = $this->getConsumeByPlayerId($val,3,1);
            $data[$parentId]['direct'][$val] = $consume;
        }
        $dailiList = $this->getUnderPlayerList($parentId, 1);
        foreach ($dailiList as $d => $vv) {
            $dailiConsume = $this->getPlayerTodayAchievements($vv, 2,'20181210');
            $data[$parentId]['daili'][$vv] = $dailiConsume;
        }

        //$data[$parentId]['self'] = $this->getConsumeByPlayerId($parentId);
