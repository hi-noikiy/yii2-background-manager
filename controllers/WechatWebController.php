...
class WechatWebController extends CommonBaseController
{
    ...
    {
        $redis = Yii::$app->redis;
        return $redis->get(RedisKey::WEB_PLAYER_ID);
    }
    ...
    public function actionIndex()
    {
        ...
        if ($request->isPost) {
//            file_put_contents('/tmp/1.log', print_r($_POST, 1), FILE_APPEND);
            $redis = Yii::$app->redis;
            $redis->set(RedisKey::WEB_PLAYER_ID, $request->post('player_id'));
            $this->redirect('wechat-web/index');
        }


        $player_id = $this->getUserId();
        if (!$player_id) {
            return $this->render('index', ['daili_num' => 0, 'player_num' => 0]);
        } else {
            $user = LobbyPlayer::find()->select(['u_id'])->from('login_db.t_lobby_player')->where(['u_id' => $player_id])->asArray()->scalar();
...
//            var_dump($user);
            ...
            $myUserInfo = $dailiModel->getById($user);
            ...
            if (!Yii::$app->params['pay_daili_forbid_switch']) {
                return $this->render('index', $this->playerAndMemberNums($player_id));
            }
        }
    }
...
//业绩
    ...
    public function actionMyBaseInfo()
    {
        ...
        if ($allUser) {
            $num = $this->playerAndMemberNums($my_user);
            //下级玩家
...
//                $tran->rollBack();
    ...
    public function actionResultsDay()
    {
        ...
        foreach ($resDate as $key => $val) {
            ...
            $returnData[$i]['rebate'] = Common::disposeStr(Yii::$app->redis->hget($dateRedisKey, $playerId) / self::REBATE_RATIO) ?: 0;
